...
async function main() {
  ...
  if (existingAdmin.rowCount === 0) {
    ...
    await pool.query(
      ...
      [adminEmail.toLowerCase(), hashed, "admin", "Admin", "0000000000", new Date(), process.env.PDPA_VERSION ?? "v1.0-2026-05-18"]
    );
  ...
  if (existingShop.rowCount === 0) {
    await pool.query(
      `INSERT INTO shop_config (shop_name, contact_email, contact_phone, tax_code, bank_account, bank_name, bank_owner_name, social_links, vat_invoice_provider, vat_invoice_config, updated_at)
  ...
  const existingBranch = await pool.query("SELECT id FROM branches WHERE name = $1", ["Chi nhánh chính"]);
