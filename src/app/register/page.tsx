  ...
  return (
    <div className="min-h-screen bg-slate-50 py-16 px-4 sm:px-6 lg:px-8">
      <main className="mx-auto w-full max-w-md rounded-3xl bg-white p-8 shadow-lg shadow-slate-200/50">
        <h1 className="mb-6 text-3xl font-semibold text-slate-900">Đăng ký tài khoản</h1>
        {success ? (
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
              ...
              <input type="text" {...register("fullName")} className={cn("w-full rounded-xl border px-4 py-3 text-slate-900 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200", errors.fullName && "border-rose-500")} />
            ...
            <div>
              ...
              <input type="tel" {...register("phone")} className={cn("w-full rounded-xl border px-4 py-3 text-slate-900 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200", errors.phone && "border-rose-500")} />
              <p className="mt-1 text-sm text-rose-600">{errors.phone?.message}</p>
            </div>

            <div className="flex items-start gap-3">
              <input id="pdpaConsented" type="checkbox" {...register("pdpaConsented")} className="mt-1 h-4 w-4 rounded border-slate-300 text-slate-700 focus:ring-slate-500" />
              <div>
                <label htmlFor="pdpaConsented" className="block text-sm font-medium text-slate-700">Tôi đồng ý với điều khoản PDPA</label>
                <p className="text-sm text-slate-500">Bạn phải đồng ý để tạo tài khoản.</p>
              </div>
            </div>
